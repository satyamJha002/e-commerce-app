...
const Header = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-base-100 shadow-sm flex justify-between items-center px-4 sm:px-8 py-4">
      {/* Logo */}
      ...
      {/* Sidebar for Mobile, Inline for Desktop */}
      <div
        ...
        } md:static md:w-auto md:h-auto md:bg-transparent md:shadow-none md:transform-none md:flex md:items-center md:gap-4`}
      >
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 p-4 md:p-0">
          ...
          <div className="relative w-full md:w-auto">
            <input
              type="text"
              className={`border-2 border-gray-600 px-3 py-1 rounded-md focus:outline-none w-full md:w-64 transition-all duration-300 ease-in-out ${
                showSearch ? "opacity-100" : "opacity-0 md:opacity-100"
              }`}
              placeholder="Search..."
            />
            <button
              className="md:hidden flex items-center gap-2 focus:outline-none absolute right-2 top-1/2 transform -translate-y-1/2"
          ...
          <details className="dropdown dropdown-hover w-full md:w-auto">
            <summary className="btn btn-ghost flex items-center gap-2 w-full md:w-auto justify-start md:justify-center">
              <FaUser />
              Account
            </summary>
      ...
      {isMenuOpen && (
        <div
          className="fixed inset-0 bg-[#00000085] bg-opacity-20 z-40 md:hidden"
          onClick={toggleMenu}
          aria-hidden="true"
        ></div>
      )}
